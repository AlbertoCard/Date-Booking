    ...
    public function success()
    {
        try {
            ...
            if (empty($stripeKey)) {
                ...
                return redirect($_ENV['APP_URL'] . '/reservas?error=configuracion');
            }
            try {
                Stripe::setApiKey($stripeKey);
            } catch (\Exception $e) {
                ...
                return redirect($_ENV['APP_URL'] . '/reservas?error=configuracion_stripe');
            }

            // Verificar que tenemos el session_id
            if (!isset($_GET['session_id'])) {
                Log::error('No se recibió session_id en success');
                return redirect($_ENV['APP_URL'] . '/reservas?error=sesion');
            }
            ...
            try {
                ...
                return redirect($_ENV['APP_URL'] . '/reservas?error=stripe_session');
            } catch (\Exception $e) {
                Log::error('Error general al recuperar la sesión de Stripe:', [
                    'message' => $e->getMessage()
                ]);
                return redirect($_ENV['APP_URL'] . '/reservas?error=stripe_session_general');
            }
            ...
            if (!isset($session->metadata->reserva_id)) {
                ...
                return redirect($_ENV['APP_URL'] . '/reservas?error=metadatos');
            }
            ...
            try {
                ...
                return redirect($_ENV['APP_URL'] . '/reservas?error=creacion_pago');
            }
            ...
            try {
                ...
                return redirect($_ENV['APP_URL'] . '/reservas?error=buscar_reserva');
            }

            if (!$reserva) {
                Log::error('No se encontró la reserva:', ['reserva_id' => $reservaId]);
                return redirect($_ENV['APP_URL'] . '/reservas?error=reserva_no_encontrada');
            }
            ...
            try {
                ...
                return redirect($_ENV['APP_URL'] . '/reservas?error=actualizacion_reserva');
            }

            // Redirigir al usuario a sus reservas
            return redirect($_ENV['APP_URL'] . "/reservas/{$userId}?success=true");
        } catch (\Exception $e) {
            ...
            return redirect($_ENV['APP_URL'] . '/reservas?error=proceso');
        }
    }
