    ...
    public function success()
    {
        try {
            ...
            if (empty($stripeKey)) {
                ...
                return redirect(config('app.url') . '/reservas?error=configuracion');
            }
            try {
                Stripe::setApiKey($stripeKey);
            } catch (\Exception $e) {
                ...
                return redirect(config('app.url'). '/reservas?error=configuracion_stripe');
            }

            // Verificar que tenemos el session_id
            if (!isset($_GET['session_id'])) {
                Log::error('No se recibió session_id en success');
                return redirect(config('app.url') . '/reservas?error=sesion');
            }
            ...
            try {
                ...
                return redirect(config('app.url') . '/reservas?error=stripe_session');
            } catch (\Exception $e) {
                Log::error('Error general al recuperar la sesión de Stripe:', [
                    'message' => $e->getMessage()
                ]);
                return redirect(config('app.url') . '/reservas?error=stripe_session_general');
            }
            ...
            if (!isset($session->metadata->reserva_id)) {
                ...
                return redirect(config('app.url') . '/reservas?error=metadatos');
            }
            ...
            try {
                ...
                return redirect(config('app.url') . '/reservas?error=creacion_pago');
            }
            ...
            try {
                ...
                return redirect(config('app.url') . '/reservas?error=buscar_reserva');
            }

            if (!$reserva) {
                Log::error('No se encontró la reserva:', ['reserva_id' => $reservaId]);
                return redirect(config('app.url') . '/reservas?error=reserva_no_encontrada');
            }
            ...
            try {
                ...
                return redirect(config('app.url') . '/reservas?error=actualizacion_reserva');
            }

            // Redirigir al usuario a sus reservas
            return redirect(config('app.url') . "/reservas/{$userId}?success=true");
        } catch (\Exception $e) {
            ...
            return redirect(config('app.url') . '/reservas?error=proceso');
        }
    }
